...
int dim = 100;
float eig=0.0;
char *c_input_dir_name, *c_input_file_U_name, *c_input_file_S_name, *c_output_path_name;
...
int run() {
    ...
    std::string output_name = std::string(c_output_path_name);
...
int main(int argc, char **argv) {
    ...
    c_output_path_name = (char*)malloc(sizeof(char) * MAX_PATH_NAME);
    ...
    if ((i = find_arg((char *)"-output-path", argc, argv)) > 0) strcpy(c_output_path_name, argv[i + 1]);
    else strcpy(c_output_path_name, "words.txt");
